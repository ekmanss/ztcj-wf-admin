...
vi.mock('../hooks/use-users-query', () => ({
  useDeleteUserMutation: () => ({
    mutateAsync: mutationMocks.deleteUser,
    isPending: false,
  }),
}))
...
describe('UsersDeleteDialog', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mutationMocks.deleteUser.mockResolvedValue({ id: MOCK_USER.id })
  })
  ...
  it('deletes the user when confirmed', async () => {
    ...
    expect(mutationMocks.deleteUser).toHaveBeenCalledOnce()
    expect(mutationMocks.deleteUser).toHaveBeenCalledWith(MOCK_USER.id)
  })
