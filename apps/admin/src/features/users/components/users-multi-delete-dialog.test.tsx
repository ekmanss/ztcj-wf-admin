...
vi.mock('../hooks/use-users-query', () => ({
  useDeleteUsersMutation: () => ({
    mutateAsync: mutationMocks.deleteUsers,
    isPending: false,
  }),
}))
...
describe('UsersMultiDeleteDialog', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mutationMocks.deleteUsers.mockResolvedValue({ count: 2 })
  })
  ...
  it('deletes selected users when confirmed', async () => {
    ...
    expect(onOpenChange).toHaveBeenCalledOnce()
    expect(onOpenChange).toHaveBeenCalledWith(false)
    expect(mutationMocks.deleteUsers).toHaveBeenCalledOnce()
